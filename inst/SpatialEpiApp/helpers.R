...
#'
fnPlotMap<-function(maparea, vecVblePintar, nombreGuardar, minlegend, maxlegend, discretedata, labelstext, labelsx, labelsy){
  ...
  sc<-scale_fill_gradient2( midpoint = 1, low = "#27408B", mid = "white", high = "#B22222"  ) 
  gt<-NULL
  leyendaquitar<-NULL
  ...
  #   valorescolor2<-valorescolor[1+ sort(unique(maparea[,vecVblePintar]))]
  ...
  # 
  #   maparea[,vecVblePintar]<-as.factor(maparea[,vecVblePintar])
  # 
  # }
  
  for (vp in vecVblePintar) {
    if(vp == "Population"){
      g <- ggplot(data = maparea,  aes(fill = Population))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
    ...
    if(vp == "Observed"){
      g <- ggplot(data = maparea,  aes(fill = Observed))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
    ...
    if(vp == "Expected"){
      g <- ggplot(data = maparea,  aes(fill = Expected))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
    ...
    if(vp == "SIR"){
      g <- ggplot(data = maparea,  aes(fill = SIR))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
    ...
    if(vp == "Risk"){
      g <- ggplot(data = maparea,  aes(fill = Risk))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
    ...
    if(vp == "LowerLimitCI"){
      g <- ggplot(data = maparea,  aes(fill = LowerLimitCI))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
    ...
    if(vp == "UpperLimitCI"){
      g <- ggplot(data = maparea,  aes(fill = UpperLimitCI))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
    ...
    if(vp == "Clusters"){
      ...
      g <- ggplot(data = maparea,  aes(fill = Clusters))  +
        geom_sf() +
        theme_bw() + 
        sc + 
        gt + 
...
fnCreateMaps<-function(datostime, vectimesvt, map, vecVblePintar, vecMinlegend, vecMaxlegend, discretedata, labelstext, labelsx, labelsy){
    ...
    maparea <- st_as_sf(map, region="idAreaInteger")
    
    maparea <- left_join(maparea, dd, by = c("idAreaInteger" = "id"))
