...
fnMakeMapsWithProgress<-function(vecVblePintar, discretedata="no", labelstext=NULL, labelsx=NULL, labelsy=NULL, allmapssamelegend="no"){
  ...
  #vecVblePintar<-c("Population","Observed","Expected","Risk")
  vecTimes<-unique(datosP$time)
  n<-length(vecTimes)

  withProgress(message = 'In progress. Trends.', value = 0, {
  ### INI Plots temporal trends. 1 plot for each vble
  for(vp in vecVblePintar){
    if(vp == "Population"){
      nombreGuardar<-paste0("TemporalTrend","Population")
      g<-ggplot(datosP, aes(x = timeformatted, y = Population, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
    ...
    if(vp == "Observed"){
      nombreGuardar<-paste0("TemporalTrend","Observed")
      g<-ggplot(datosP, aes(x = timeformatted, y = Observed, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
        xlab("") + theme_minimal() + theme(legend.position="none")
        png(paste("plots/",nombreGuardar,".png",sep=""), width=5, height=5, units="in", res=300)
        plot(g)
      dev.off()
    ...
    if(vp == "Expected"){
      nombreGuardar<-paste0("TemporalTrend","Expected")
      g<-ggplot(datosP, aes(x = timeformatted, y = Expected, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
       xlab("") + theme_minimal() + theme(legend.position="none")
      png(paste("plots/",nombreGuardar,".png",sep=""), width=5, height=5, units="in", res=300)
      plot(g)
      dev.off()
    ...
    if(vp == "SIR"){
     nombreGuardar<-paste0("TemporalTrend","SIR")
     g<-ggplot(datosP, aes(x = timeformatted, y = SIR, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
       xlab("") + theme_minimal() + theme(legend.position="none")
     png(paste("plots/",nombreGuardar,".png",sep=""), width=5, height=5, units="in", res=300)
     plot(g)
     dev.off()
    ...
    if(vp == "Risk"){
      nombreGuardar<-paste0("TemporalTrend","Risk")
      g<-ggplot(datosP, aes(x = timeformatted, y = Risk, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
        xlab("") + theme_minimal() + theme(legend.position="none")
      png(paste("plots/",nombreGuardar,".png",sep=""), width=5, height=5, units="in", res=300)
      plot(g)
      dev.off()
    ...
    if(vp == "LowerLimitCI"){
      nombreGuardar<-paste0("TemporalTrend","LowerLimitCI")
      g<-ggplot(datosP, aes(x = timeformatted, y = LowerLimitCI, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
        xlab("") + theme_minimal() + theme(legend.position="none")
      png(paste("plots/",nombreGuardar,".png",sep=""), width=5, height=5, units="in", res=300)
      plot(g)
      dev.off()
    ...
    if(vp == "UpperLimitCI"){
      nombreGuardar<-paste0("TemporalTrend","UpperLimitCI")
      g<-ggplot(datosP, aes(x = timeformatted, y = UpperLimitCI, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
        xlab("") + theme_minimal() + theme(legend.position="none")
      png(paste("plots/",nombreGuardar,".png",sep=""), width=5, height=5, units="in", res=300)
      plot(g)
      dev.off()
...
fnDetectClustersSatscanInstalled<-function(executablepath){
  ...
  withProgress(message = 'In progress. Trends.', value = 0, {
    ...
    ### INI Plots temporal trends. 1 plot for each vble
    for(vp in vecVblePintar){
      nombreGuardar<-paste0("TemporalTrend",vp)
      #valor<-vecVblePintar[vp]
      datosPNA<-datosP
      datosPNA[which(datosPNA$Clusters==0),"Clusters"]<-NA
      g<-ggplot(datosPNA, aes(x = timeformatted, y = Clusters, group = id)) + geom_point(color="#27408B") + geom_line(color="#27408B") +
...
fnEstimateRiskINLAInstalled<-function(){
  ...
  withProgress(message = 'In progress. Estimate risk.', value = 0, {
   datosrisk <-fnEstimateRisk(rv$datoswithvaluesforeachidandtime,rv$map,rv$usedarealcovs)
    rv$datoswithvaluesforeachidandtime <- datosrisk[[1]]
  })
...
observe({
  ...
  tempdirname <- paste0(dirname(shpdf$datapath[1]), "/", shpdf$name[1])
  
  file.rename(shpdf$datapath[1],tempdirname)
  
  localdirname <- paste0(getwd(), "/data/", shpdf$name[1])
  file.copy(tempdirname, paste0(getwd(), "/data/", shpdf$name[1]))
  
 
  map <- shapefile(localdirname)
...
observe({
  ...
  csv_pattern <- "([/][:digit:]+[.csv]+)"
  
  name_csv_temp <- str_extract(input$filedata$datapath, csv_pattern)
  
  file.copy(input$filedata$datapath, getwd())
  
  path_csv_local <- paste0(getwd(), "", name_csv_temp)
  path_csv_local_real <- paste0(getwd(), "/data/", input$filedata$name)
  
  file.rename(path_csv_local, path_csv_local_real)
  
  if(file.exists(path_csv_local)) {
    file.remove(path_csv_local)
  }
  rv$datosOriginal<-read.csv(path_csv_local_real)
})
...
if(input$useSampleData){
  rv$datosOriginal<-read.csv("data/dataohiocomplete.csv")
  #rv$map<-readShapePoly("data/Ohio/fe_2007_39_county/fe_2007_39_county", delete_null_obj=TRUE)
  rv$map<-shapefile("data/fe_2007_39_county.shp")
  }else{
...
pal <- colorNumeric(palette = "Blues", domain = values)
...
leaflet(mapFiltered) %>%
  ...
  leaflet::addLegend("bottomright",
                     ...
                     opacity = 1, title = input$vblePintar,
...
tablaclusterscol<- reactive({
  ...
  if(ncol(tabla)>1){
   ...
   #Write fechas
    ...
    fechassfinal<-tabla[,6]
    fechasefinal<-tabla[,7] 
    ...
    fechastabla <- make_date(fechas[1]) 
    fechass<- fechastabla[1]  + years(round(fechastart[1]))
    fechase<- fechastabla[1]  + years(fechaend[1])
    ...
    for(i in 1:length(vecIdclusters)){
      ...
      fechass<- fechastabla[1]  + years(round(fechastart[i]))
      fechassfinal[i] <- format(fechass, "%Y")
      
      fechase <- fechastabla[1]  + years(fechaend[i])
      fechasefinal[i] <- format(fechase, "%Y")
    }
    
    tabla[,6]<- fechassfinal
    tabla[,7]<- fechasefinal
    tabla[,16]<- fechassfinal
    tabla[,17]<- fechasefinal
  }
  ...
  tabla <- tabla %>% dplyr::select("Cluster", "Central area","No. areas","Start date","End date","Risk in / Risk out","LLR", "p-value","Areas")
